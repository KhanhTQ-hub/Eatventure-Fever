...
namespace Hellmade.Net
{
    ...
    /// <summary>
    /// NetCheck error information container
    /// </summary>
    public class ErrorInfo
    {
        /// <summary>
        /// Whether there was an error with the network
        /// </summary>
        ...
        /// <summary>
        /// Whether there was an HTTP error
        /// </summary>
        ...
        public ErrorInfo(bool isNetworkError, bool isHTTPError, string errorMsg)
        {
            ...
            IsError = IsNetworkError || IsHTTPError;
            ErrorMsg = errorMsg;
        }

        public override string ToString()
        {
            return string.Format("NetworkError: {0}, HTTPError: {1}, Error: {2}", IsNetworkError.ToString(), IsHTTPError.ToString(), ErrorMsg);
        }
    }
    ...
    public class EazyNetChecker : MonoBehaviour
    {
            ...
            get
            {
                switch (Status)
                {
                    case NetStatus.NoDNSConnection:
                        {
                            return CheckIntervalOnNoConnection;
                        }
                    default:
                        {
                            return CheckIntervalNormal;
                        }
                }
            }
        }
        ...
        {
            get { return Instance.checkIntervalNormal; }
            set
            {
                Instance.checkIntervalNormal = Mathf.Max(3, value);
            }
        }

        /// <summary>
        /// How often an internet check will be performed (if a continuous check is started) when there is no connection. The minimum is 3 seconds. If less than the minimum is given, it will automatically set on the minimum.
        /// </summary>
        public static float CheckIntervalOnNoConnection
        {
            get { return Instance.checkIntervalOnNoConnection; }
            set
            {
                Instance.checkIntervalOnNoConnection = Mathf.Max(3, value);
            }
        }
        ...
        /// <summary>
        /// Whether to continue a continuous check after a timeout 
        /// </summary>
        public static bool ContinueCheckAfterTimeout { get { return Instance.continueCheckAfterTimeout; } set { Instance.continueCheckAfterTimeout = value; } }
        ...
        /// <summary>
        /// Information about any network error
        /// </summary>
        public static ErrorInfo ErrorInfo { get; private set; } = new ErrorInfo();
        ...
        [SerializeField]
        private bool continueCheckAfterTimeout = true;
        [SerializeField]
        private float checkIntervalNormal = 10;
        [SerializeField]
        ...
        private void Update()
        {
            ...
            {
                if (keepChecking && ContinueCheckAfterTimeout)
                {
                    StartConnectionCheck(stopOnSuccess, true);
                }
                else
                {
                    StopConnectionCheck();
...
//                    Debug.Log("[Eazy NetChecker] Internet check started");
                ...
                ErrorInfo = GetSelectedMethod().GetErrorInfo();
                ...
                //{
                    ...
                    {
                        //Debug.Log("OnConnectionStatusChanged: " + Status);
                ...
                nextCheckTime = keepChecking ? Time.time + CheckInterval : nextCheckTime;

                yield return keepChecking ? new WaitForSeconds(CheckInterval) : null;
            } while (keepChecking);
