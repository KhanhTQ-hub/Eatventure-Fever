 ...
 */
 ...
 namespace Mosframe {
    ...
    public abstract class DynamicScrollView : UIBehaviour {
        ...
        {
            get => this.scrollRect.content;
            set => this.scrollRect.content = value;
        }
        ...
        public int             NextInsertItemNo => nextInsertItemNo;
        ...
        public void ScrollToHalfItem(int itemIndex)
        {
            var totalLen = this.contentSize;
            var itemLen = totalLen / this.totalItemCount;
            var pos = itemLen * itemIndex;
            this.contentAnchoredPosition = -pos + (itemLen / 2);
        }    
        ...
        {
            var lst = GetActiveItems();
            for (int i = lst.Count - 1; i >= 0; i--)
            {
                var view = lst[i].GetComponent<IDynamicScrollViewItem>();
                if (view != null) lst[i].gameObject.transform.SetSiblingIndex(view.getIndex());
            }
        }
        ...
        public bool isIntComplete = false;
